...
export default function EditProductModal({ product, onClose, onSave, userEmail }: EditProductModalProps) {
  ...
  const [subcategories, setSubcategories] = useState<Array<{subcategory_id: number, name: string, category_id: number}>>([]);
  const [productTypes, setProductTypes] = useState<Array<{product_type_id: number, name: string, subcategory_id: number}>>([]);
  ...
  const [subcategory, setSubcategory] = useState(product.subcategory_id?.toString() || '');
  const [product_type, setProduct_type] = useState(product.product_type_id?.toString() || '');
  ...
  useEffect(() => {
    if (category) {
      fetch(`/api/taxonomy/subcategories?category_id=${category}`)
  ...
  useEffect(() => {
    if (subcategory) {
      fetch(`/api/taxonomy/product-types?subcategory_id=${subcategory}`)
        .then(response => response.json())
        .then(data => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const productData = {
        ...
        subcategory_id: subcategory ? parseInt(subcategory) : undefined,
        product_type_id: product_type ? parseInt(product_type) : undefined,
          ...
          <div className="space-y-4">
            {/* Product Name */}
              ...
              <input
                ...
                {/* Category */}
                <div className="space-y-1">
                  <label className="mb-1 block text-xs text-subheader">
                  ...
                  <select
                    ...
                    className={`w-full rounded-md border px-3 py-2 text-[12px] text-gray-600 focus:outline-none focus:ring-2 focus:ring-primary-500 appearance-none cursor-pointer disabled:bg-gray-100 disabled:cursor-not-allowed ${
                      validationErrors.category ? 'border-red-500' : 'border-gray-300'
                    } bg-white`}
                  >
                    <option value="">{loadingTaxonomy ? 'Loading categories...' : 'Select option'}</option>
                    {categories.map((cat) => (
                ...
                <div className="space-y-1">
                  ...
                  {validationErrors.subcategory && (
                    <p className="text-xs text-red-500">{validationErrors.subcategory}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    >
                      <option value="">Select option</option>
                      {productTypes.map((pt) => (
                        <option key={pt.product_type_id} value={pt.product_type_id.toString()}>
                          {pt.name}
                        </option>
                      ))}
                    </select>
