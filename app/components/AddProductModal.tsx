...
export default function AddProductModal({ onClose, onSave, userEmail }: AddProductModalProps) {
  ...
  const validateForm = () => {
    ...
    if (!category1.trim()) {
      ...
      const selectedPath = [category1, category2, category3, category4, category5, category6].filter(cat => cat.trim());
      
      // Require the next level if its options exist but no selection was made
      if (cat2Options.length > 0 && !category2.trim()) {
        errors.category2 = 'Please select a subcategory';
      } else if (cat3Options.length > 0 && !category3.trim()) {
        errors.category3 = 'Please select a subcategory';
      } else if (cat4Options.length > 0 && !category4.trim()) {
        errors.category4 = 'Please select a subcategory';
      } else if (cat5Options.length > 0 && !category5.trim()) {
        errors.category5 = 'Please select a subcategory';
      } else if (cat6Options.length > 0 && !category6.trim()) {
  ...
  // Load Category 1 options from API
  useEffect(() => {
    let alive = true;
    (async () => {
      try {
  ...
  useEffect(() => {
    setCategory2(''); setCategory3(''); setCategory4(''); setCategory5(''); setCategory6('');
  ...
  useEffect(() => {
    setCategory3(''); setCategory4(''); setCategory5(''); setCategory6('');
  ...
  useEffect(() => {
    setCategory4(''); setCategory5(''); setCategory6('');
